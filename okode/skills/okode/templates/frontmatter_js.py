"""
oKode Frontmatter Generator — JavaScript/TypeScript Files

Generates the oKode frontmatter comment block that is inserted at the top
of JS/TS source files using JSDoc-style block comments. This frontmatter
provides at-a-glance understanding of a file's role and runtime relationships.
"""


def generate_js_frontmatter(node_data: dict, edges: list) -> str:
    """Generate oKode frontmatter comment block for a JS/TS file.
    ...
    Returns:
        A string containing the complete JSDoc-style frontmatter block,
...
def strip_existing_frontmatter(source: str) -> str:
    """Remove existing oKode frontmatter from a JS/TS source string.

    Looks for the JSDoc block containing 'oKode Graph Metadata' and removes
    the entire block comment.

    Args:
        source: The full source code of the JS/TS file.
    ...
    while i < len(lines):
        line = lines[i]
        # Detect start of oKode frontmatter block
        if line.strip() == "/**" and i + 2 < len(lines):
            # Look ahead for the oKode marker
            lookahead = "\n".join(lines[i : i + 5])
            if "oKode Graph Metadata" in lookahead:
                in_frontmatter = True
                i += 1
                continue
        if in_frontmatter:
            if line.strip() == "*/":
                in_frontmatter = False
                i += 1
                # Skip blank line after closing comment
...
def inject_frontmatter(source: str, node_data: dict, edges: list) -> str:
    """Inject or replace oKode frontmatter in a JS/TS source file.

    If existing frontmatter is present, it is replaced. Otherwise, the
    frontmatter is prepended to the file, preserving any 'use strict'
    directive or initial comments.

    Args:
        source: The full source code of the JS/TS file.
        node_data: Node metadata dictionary (see generate_js_frontmatter).
        edges: List of edge dictionaries (see generate_js_frontmatter).
    ...
    frontmatter = generate_js_frontmatter(node_data, edges)

    # Preserve 'use strict' or initial pragmas
    ...
    for i, line in enumerate(lines):
        stripped = line.strip()
        if i == 0 and (stripped == "'use strict';" or stripped == '"use strict";'):
